...
def calculate_paths(shape: (int, int), point: (int, int)) -> int: #
    ...
    номирация  shape начинается с 1 номерация point начинается с 0
    :param shape: tuple of rows and cols (each from 1 to rows/cols)
    :param point: desired point for horse
    :return: count of paths from (1, 1) to (point[0], point[1]) (numerating from 0, so (0, 0) - left bottom tile)
    """
    rows = shape[0] # колличество строк лтсчёта с 1
    cols = shape[1] # колличество столбцов отсчёта с 1
    ...
    def get_steps(i, j):
        ...
        if not 0 <= i < rows:  # выпадаю за границы поля для строк
            return 0
        if not 0 <= j < cols:  # выпадаю за границы для строк
            return 0
